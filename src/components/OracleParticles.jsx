...
const OracleParticles = ({ isHovered }) => {
  ...
  useEffect(() => {
    ...
    const particles = []
    const octagonSize = 160
    const innerOctagonSize = octagonSize * 0.6

    const createOctagonPoints = () => {
      const points = []
      const centerX = canvas.width / 2
      const centerY = canvas.height / 2
      
      // Создаем точки внешнего и внутреннего восьмиугольников
      const createVertices = (size) => {
        const vertices = []
        for (let i = 0; i < 8; i++) {
          const angle = (i * Math.PI) / 4 - Math.PI / 8 // Начинаем с верхней точки
          vertices.push({
      ...
      const outerVertices = createVertices(octagonSize)
      const innerVertices = createVertices(innerOctagonSize)
      ...
      for (let i = 0; i < 400; i++) {
        ...
        const r = Math.random() * (octagonSize - innerOctagonSize) + innerOctagonSize
    ...
    const octagonPoints = createOctagonPoints()
    particles.push(...octagonPoints)
    ...
    const animate = () => {
      ...
      if (progressRef.current > 0) {
        const gradient = ctx.createRadialGradient(
          ...
          canvas.width/2, canvas.height/2, octagonSize
        )
        gradient.addColorStop(0, `rgba(255, 0, 102, ${0.1 * progressRef.current})`)
      ...
      particles.forEach(particle => {
        ...
        ctx.fillStyle = `rgba(255, 0, 102, ${particle.opacity * progressRef.current})`
