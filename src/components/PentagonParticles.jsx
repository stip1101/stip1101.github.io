...
const PentagonParticles = ({ isHovered }) => {
  ...
  useEffect(() => {
    ...
    const particles = []
    const pentagonSize = 160
    const innerPentagonSize = pentagonSize * 0.6
    ...
    const createPentagonPoints = () => {
      ...
      // Создаем точки внешнего и внутреннего пятиугольников
      const createVertices = (size) => {
        const vertices = []
        for (let i = 0; i < 5; i++) {
          const angle = (i * 2 * Math.PI) / 5 - Math.PI / 2 // Начинаем с верхней точки
          vertices.push({
      ...
      const outerVertices = createVertices(pentagonSize)
      const innerVertices = createVertices(innerPentagonSize)
      ...
      // Добавляем частицы между пятиугольниками
      for (let i = 0; i < 250; i++) {
        const angle = Math.random() * Math.PI * 2
        const radius = (pentagonSize - innerPentagonSize) * Math.random() + innerPentagonSize
    ...
    const pentagonPoints = createPentagonPoints()
    particles.push(...pentagonPoints)
    ...
    const animate = () => {
      ...
      if (progressRef.current > 0) {
        const gradient = ctx.createRadialGradient(
          ...
          canvas.width/2, canvas.height/2, pentagonSize
        )
        gradient.addColorStop(0, `rgba(0, 240, 255, ${0.1 * progressRef.current})`)
      ...
      particles.forEach(particle => {
        ...
        ctx.fillStyle = `rgba(0, 240, 255, ${particle.opacity * progressRef.current})`
...
export default PentagonParticles 